...
void S3FileSystem::DownloadObject(const String &file_path){
    ...
    // Create S3 client.
    Aws::S3::S3Client client = GetClient();
    ...
    if (outcome.IsSuccess()) {
        ...
        Aws::OFStream outStream(file_path,
                                std::ios_base::out | std::ios_base::binary);
        if (!outStream.is_open()) {
            Aws::ShutdownAPI(options);
            UnrecoverableError("Error: unable to open file, '"+file_path+"'.");
        } else {
        ...
        Aws::ShutdownAPI(options);
        UnrecoverableError("Error: getObject: "+err.GetExceptionName()+": "+err.GetMessage());
    }
    Aws::ShutdownAPI(options);
}
...
void S3FileSystem::UploadObject(const String &file_path){
    ...
    if (outcome.IsSuccess()) {
        ...
        Aws::ShutdownAPI(options);
        UnrecoverableError("Error: putObject: "+outcome.GetError().GetExceptionName()+": "+outcome.GetError().GetMessage());
    }
    Aws::ShutdownAPI(options);
}
...
void S3FileSystem::RemoveObject(const String &file_path){
    ...
    if (outcome.IsSuccess()) {
        ...
        Aws::ShutdownAPI(options);
        UnrecoverableError("Error: deleteObject: "+err.GetExceptionName()+": "+err.GetMessage());
    }
    Aws::ShutdownAPI(options);
}
...
void S3FileSystem::CopyObject(const String &src_file_path, const String &dst_file_path){
    ...
    request.WithBucket(bucket_name)
        .WithKey(dst_objectname)
        .WithCopySource(bucket_name + "/" + src_objectname);

    Aws::S3::Model::CopyObjectOutcome outcome =
        client.CopyObject(request);
    if (outcome.IsSuccess()) {
        std::cout << "Copied the object with the key, '" << src_objectname
                  << "', to the key, '" << dst_objectname
                  << ", in the bucket, '" << bucket_name << "'." << std::endl;
    } else {
        auto err = outcome.GetError();
        Aws::ShutdownAPI(options);
        UnrecoverableError("Error: copyObject: "+err.GetExceptionName()+": "+err.GetMessage());
    }
